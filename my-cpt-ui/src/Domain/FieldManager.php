...
use MyCptUi\Domain\Model\FieldGroup;
...
class FieldManager {
    ...
    public function __construct( OptionsRepository $repository ) {
        ...
        add_action( 'admin_post_my_cpt_ui_save_field_group', [ $this, 'handle_save' ] );
        add_action( 'admin_post_my_cpt_ui_delete_field_group', [ $this, 'handle_delete' ] );
        add_action( 'add_meta_boxes', [ $this, 'add_meta_boxes' ] );
        add_action( 'save_post', [ $this, 'save_meta' ], 10, 2 );
    }

    public function register_fields(): void {
        $groups = apply_filters( 'my_cpt_ui_field_groups', $this->repository->get_config()['field_groups'] );
    ...
    public function handle_save(): void {
        ...
        check_admin_referer( 'my_cpt_ui_save_field_group' );
    ...
    public function handle_delete(): void {
        ...
        check_admin_referer( 'my_cpt_ui_delete_field_group' );
    ...
    public function add_meta_boxes(): void {
        ...
        foreach ( $groups as $data ) {
            ...
            foreach ( $group->locations as $post_type ) {
                add_meta_box(
                    'my_cpt_ui_' . $group->group_id,
                    esc_html( $group->title ),
                    function( $post ) use ( $group ) {
                        wp_nonce_field( 'my_cpt_ui_fields_' . $group->group_id, 'my_cpt_ui_fields_nonce_' . $group->group_id );
                        echo '<div class="my-cpt-ui-metabox">';
                        foreach ( $group->fields as $field ) {
                            $value = get_post_meta( $post->ID, $field['name'], true );
                            $value = $value !== '' ? $value : ( $field['default_value'] ?? '' );
                            echo '<div class="my-cpt-ui-field">';
    ...
    private function render_input( array $field, $value ): void {
        ...
        switch ( $type ) {
            ...
            case 'checkbox':
                ...
                echo '<label><input type="checkbox" name="' . $name . '" value="1" ' . $checked . ' /> ' . esc_html__( 'Required', 'my-cpt-ui' ) . '</label>';
                break;
            default:
    ...
    public function save_meta( int $post_id, $post ): void {
        ...
        foreach ( $groups as $data ) {
            ...
            $nonce_name = 'my_cpt_ui_fields_nonce_' . $group->group_id;
            if ( ! isset( $_POST[ $nonce_name ] ) || ! wp_verify_nonce( $_POST[ $nonce_name ], 'my_cpt_ui_fields_' . $group->group_id ) ) {
                continue;
            }
